...
//        Log.d(Constants.TAG_DASHBOARD,"mCenterY: " + mCenterY);       //TODO 不適用未被約束的ConstraintLayout
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i <= mSection; i++) {
            ...
            p = getCoordinatePoint(mRadius - mLength2, alpha);
...
//        }
        ...
        //畫指針
        final float theta = mStartAngle + mSweepAngle * (mVelocity - mMin) / (mMax - mMin); // 指针与水平线夹角
        ...
        float[] p1 = getCoordinatePoint(mPlRadius, theta);                     //TODO 長指針
        canvas.drawLine(p1[0], p1[1], mCenterX, mCenterY, mPaint);
        float[] p2 = getCoordinatePoint(mPsRadius, theta + 180);        //TODO 短指針
    ...
    private void drawDigitalTube(Canvas canvas, int num, int xoffset) {     //TODO 速度
        ...
        final int lx = dp2px(3);                  //TODO 數字長度
        final int ly = dp2px(6);                  //TODO 數字高度
        final int gap = dp2px(2);                  //TODO 數字內寬
        // 1
